...
ed.data=data.frame(site=rep(seq(1,17),37),ED=ed.1,Year=rep(1978:2014,each=17)) # convert to data frame
ed.data$ED<-scale(ed.data$ED, center = TRUE, scale = TRUE) # scale and center data

ed <- matrix(NA,nrow=17,ncol=37) # create empty matrix of size sites x years
...
pland.out<-run.jags(data=jags.data,inits=inits,monitor=params,
              model="pland threshold model.txt",
