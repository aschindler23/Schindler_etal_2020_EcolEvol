...
# load NOBO count data
nobo.ini<-read.csv("NOBO_count_data.csv")

# input first and second visits
count.idx=rep(NA,74) # create vector of NAs
count.1=9999 # create 9999 placeholder

for(j in 1:21){ 
  sub.idx=subset(nobo.ini,nobo.ini$Year==j+1996) # subset years to 1997-2017
  ...
  count.idx=rep(NA,74) # reset to empty vector
}
count.1=count.1[-1] # drop initial 9999 placeholder

nobo.data=data.frame(route=rep(seq(1,74),21),count=count.1,Year=rep(1997:2017,each=74)) # combine two visits into single data frame

# rearrange into a matrix
y <- matrix(NA,nrow=74,ncol=21) # create empty matrix of size routes x years
for(k in 1:(length(y[1,]))){ # fill matrix with values from data frame
  sel.rows <- nobo.data$Year == k+1996
...
pdsi.idx=rep(NA,74) # create empty vector
pdsi.1=9999 # create 9999 placeholder

for(j in 1:21){
  sub.idx=subset(pdsi.ini,pdsi.ini$Year==j+1996) # subset years to 1997-2017
  sub.order=sub.idx[order(sub.idx$Route),] # order by route
  ...
  pdsi.idx=rep(NA,74) # reset to empty vector
}
pdsi.1=pdsi.1[-1] # drop initial 9999 placeholder

pdsi.data=data.frame(route=rep(seq(1,74),21),PDSI=pdsi.1,Year=rep(1997:2017,each=74)) # convert to data frame
pdsi.data$PDSI<-scale(pdsi.data$PDSI, center = TRUE, scale = TRUE) # scale and center data

pdsi <- matrix(NA,nrow=74,ncol=21) # create empty matrix of size routes x years
for(k in 1:(length(pdsi[1,]))){ # fill matrix with values from data frame
  sel.rows <- pdsi.data$Year == k+1996
...
pcp.idx=rep(NA,74) # create empty vector
pcp.1=9999 # create 9999 placeholder

for(j in 1:21){
  sub.idx=subset(pcp.ini,pcp.ini$Year==j+1996) # subset years to 1997-2017
  sub.order=sub.idx[order(sub.idx$Route),] # order by route
  ...
  pcp.idx=rep(NA,74) # reset to empty vector
}
pcp.1=pcp.1[-1] # drop initial 9999 placeholder

pcp.data=data.frame(route=rep(seq(1,74),21),PCP=pcp.1,Year=rep(1997:2017,each=74)) # convert to data frame
pcp.data$PCP<-scale(pcp.data$PCP, center = TRUE, scale = TRUE) # scale and center data

pcp <- matrix(NA,nrow=74,ncol=21) # create empty matrix of size routes x years
for(k in 1:(length(pcp[1,]))){ # fill matrix with values from data frame
  sel.rows <- pcp.data$Year == k+1996
...
tmax.idx=rep(NA,74) # create empty vector
tmax.1=9999 # create 9999 placeholder

for(j in 1:21){
  sub.idx=subset(tmax.ini,tmax.ini$Year==j+1996) # subset years to 1997-2017
  sub.order=sub.idx[order(sub.idx$Route),] # order by route
  ...
  tmax.idx=rep(NA,74) # reset to empty vector
}
tmax.1=tmax.1[-1] # drop initial 9999 placeholder

tmax.data=data.frame(route=rep(seq(1,74),21),TMAX=tmax.1,Year=rep(1997:2017,each=74)) # convert to data frame
tmax.data$TMAX<-scale(tmax.data$TMAX, center = TRUE, scale = TRUE) # scale and center data

tmax <- matrix(NA,nrow=74,ncol=21) # create empty matrix of size routes x years
for(k in 1:(length(tmax[1,]))){ # fill matrix with values from data frame
  sel.rows <- tmax.data$Year == k+1996
...
tmin.idx=rep(NA,74) # create empty vector
tmin.1=9999 # create 9999 placeholder

for(j in 1:21){
  sub.idx=subset(tmin.ini,tmin.ini$Year==j+1996) # subset years to 1997-2017
  sub.order=sub.idx[order(sub.idx$Route),] # order by route
  ...
  tmin.idx=rep(NA,74) # reset to empty vector
}
tmin.1=tmin.1[-1] # drop initial 9999 placeholder

tmin.data=data.frame(route=rep(seq(1,74),21),TMIN=tmin.1,Year=rep(1997:2017,each=74)) # convert to data frame
tmin.data$TMIN<-scale(tmin.data$TMIN, center = TRUE, scale = TRUE) # scale and center data

tmin <- matrix(NA,nrow=74,ncol=21) # create empty matrix of size route x years
for(k in 1:(length(tmin[1,]))){ # fill matrix with values from data frame
  sel.rows <- tmin.data$Year == k+1996
...
grass.idx=rep(NA,74) # create empty vector
grass.1=9999 # create 9999 placeholder

for(j in 1:21){
  sub.idx=subset(grass.ini,grass.ini$Year==j+1996) # subset years to 1997-2017
  sub.order=sub.idx[order(sub.idx$Route),] # order by route
  ...
  grass.idx=rep(NA,74) # reset to empty vector
}
grass.1=grass.1[-1] # drop initial 9999 placeholder

grass.data=data.frame(route=rep(seq(1,74),21),grass=grass.1,Year=rep(1997:2017,each=74)) # convert to data frame
grass.data$grass<-scale(grass.data$grass, center = TRUE, scale = TRUE) # scale and center data

grass <- matrix(NA,nrow=74,ncol=21) # create empty matrix of size routes x years
for(k in 1:(length(grass[1,]))){ # fill matrix with values from data frame
  sel.rows <- grass.data$Year == k+1996
...
ed.idx=rep(NA,74) # create empty vector
ed.1=9999 # create 9999 placeholder

for(j in 1:21){
  sub.idx=subset(ed.ini,ed.ini$Year==j+1996) # subset years to 1997-2017
  sub.order=sub.idx[order(sub.idx$Route),] # order by route
  ...
  ed.idx=rep(NA,74) # reset to empty vector
}
ed.1=ed.1[-1] # drop initial 9999 placeholder

ed.data=data.frame(route=rep(seq(1,74),21),ED=ed.1,Year=rep(1997:2017,each=74)) # convert to data frame
ed.data$ED<-scale(ed.data$ED, center = TRUE, scale = TRUE) # scale and center data

ed <- matrix(NA,nrow=74,ncol=21) # create empty matrix of size routes x years
for(k in 1:(length(ed[1,]))){ # fill matrix with values from data frame
  sel.rows <- ed.data$Year == k+1996
...
save(grass.out, file="nobo_grass_3k.RData")
...
save(ed.out, file="nobo_ed_3k.RData")
